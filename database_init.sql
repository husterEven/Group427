...
CREATE TABLE zone (
    ...
    created_at DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (zone_id),
    UNIQUE KEY uk_zone_section (zone_name, section_id),
...
CREATE TABLE comment (
    ...
    like_count        INT      NOT NULL DEFAULT 0      COMMENT '点赞数',
    audit_status      INT      NOT NULL DEFAULT 1      COMMENT '审核状态（0待审核 1通过 2驳回）',
    is_deleted        TINYINT(1) NOT NULL DEFAULT 0    COMMENT '软删除标记（0正常 1已删除）',
...
CREATE TABLE vote_post (
    ...
    options_json TEXT      DEFAULT NULL            COMMENT '投票选项JSON（如["选项A","选项B"]）',
    PRIMARY KEY (vote_id),
...
-- | post_id        | BIGINT       | NULL, FOREIGN KEY → post                   | NULL                | 所属帖子ID（头像附件时为空） |
-- | user_id        | BIGINT       | NULL, FOREIGN KEY → user                   | NULL                | 上传用户ID |
...
CREATE TABLE attachment (
    ...
    post_id       BIGINT       NULL                    COMMENT '所属帖子ID（外键，关联post表，头像附件时为空）',
    user_id       BIGINT       NULL                    COMMENT '上传用户ID（外键，关联user表）',
    ...
    KEY idx_user_id      (user_id),
    KEY idx_audit_status (audit_status),
    CONSTRAINT fk_att_post FOREIGN KEY (post_id) REFERENCES post (post_id) ON DELETE SET NULL,
    CONSTRAINT fk_att_user FOREIGN KEY (user_id) REFERENCES user (user_id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='附件表（一个帖子可包含多个附件，也支持头像上传）';
...
CREATE TABLE user_punishment (
    ...
    CONSTRAINT fk_pun_user     FOREIGN KEY (user_id)     REFERENCES user (user_id) ON DELETE CASCADE,
    CONSTRAINT fk_pun_operator FOREIGN KEY (operator_id) REFERENCES user (user_id) ON DELETE CASCADE
...
-- ====================================================================================================
-- 表 26：post_like（帖子点赞表）—— 记录用户对帖子的点赞关系，支持Toggle（*新增*）
-- ====================================================================================================
CREATE TABLE post_like (
    like_id    BIGINT   NOT NULL AUTO_INCREMENT COMMENT '点赞ID（主键，自增）',
    post_id    BIGINT   NOT NULL                COMMENT '帖子ID（外键，关联post表）',
    user_id    BIGINT   NOT NULL                COMMENT '点赞用户ID（外键，关联user表）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='帖子点赞表（新增，支持Toggle点赞/取消）';


-- ====================================================================================================
-- 表 27：comment_like（评论点赞表）—— 记录用户对评论的点赞关系，支持Toggle（*新增*）
-- ====================================================================================================
CREATE TABLE comment_like (
    like_id    BIGINT   NOT NULL AUTO_INCREMENT COMMENT '点赞ID（主键，自增）',
    comment_id BIGINT   NOT NULL                COMMENT '评论ID（外键，关联comment表）',
    user_id    BIGINT   NOT NULL                COMMENT '点赞用户ID（外键，关联user表）',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '点赞时间',
    PRIMARY KEY (like_id),
    ...
    CONSTRAINT fk_cl_user    FOREIGN KEY (user_id)    REFERENCES user (user_id)    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='评论点赞表（新增，支持Toggle点赞/取消）';
...
-- 为 vote_post 表添加 options_json 列（如果尚未添加）
ALTER TABLE vote_post ADD COLUMN IF NOT EXISTS options_json TEXT DEFAULT NULL COMMENT '投票选项JSON（如["选项A","选项B"]）';
